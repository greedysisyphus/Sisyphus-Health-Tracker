...
function Trends({ history }: { history: DailyOverview[] }) {
  ...
  return <>
    ...
    {!days.length ? <p className="empty">最近 7 天尚無可分析的紀錄。</p> : <>
      ...
      <section className="weekly-goals"><h2>目標達成率</h2><div><GoalProgress label={`蛋白質 ${targetRangeText(targets.proteinG, "g")}`} value={completed(day => isInRange(day.total.proteinG, targets.proteinG))} total={days.length} /><GoalProgress label={`水分 ${targetRangeText(targets.waterMl, "ml")}`} value={completed(day => isInRange(day.waterMl, targets.waterMl))} total={days.length} /><GoalProgress label={`纖維 ≥ ${targets.fiberG} g`} value={completed(day => day.total.fiberG >= targets.fiberG)} total={days.length} /><GoalProgress label={`鈉 ≤ ${targets.sodiumMg} mg`} value={completed(day => day.total.sodiumMg <= targets.sodiumMg)} total={days.length} /></div></section>
