...
type RangeTargetKey = "caloriesKcal" | "proteinG" | "carbsG" | "fatG" | "waterMl";
type HealthTargets = { caloriesKcal: TargetRange; proteinG: TargetRange; carbsG: TargetRange; fatG: TargetRange; waterMl: TargetRange; fiberG: number; sodiumMg: number };
const defaultTargets: HealthTargets = { caloriesKcal: { min: 1900, max: 2000 }, proteinG: { min: 120, max: 150 }, carbsG: { min: 170, max: 210 }, fatG: { min: 55, max: 70 }, waterMl: { min: 2800, max: 3200 }, fiberG: 30, sodiumMg: 2000 };
...
const isInRange = (value: number, target: TargetRange) => value >= target.min && value <= target.max;
const targetRangeText = (target: TargetRange, unit: string) => `${target.min}–${target.max} ${unit}`;
const rangeStatus = (value: number, target: TargetRange, unit: string) => value < target.min ? { className: "remaining", text: `距理想區間還差 ${Math.ceil(target.min - value)} ${unit}` } : value > target.max ? { className: "over", text: `高於建議上限 ${Math.ceil(value - target.max)} ${unit}` } : { className: "remaining", text: "落在理想區間" };
const profileNumber = (profile: Record<string, unknown>, key: string, fallback: number) => typeof profile[key] === "number" && Number.isFinite(profile[key]) && profile[key] >= 0 ? profile[key] : fallback;
...
export default function Home() {
  ...
  const loadDaily = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setEntries(nextEntries); setWaterMl(daily?.waterMl ?? 0); setWeightKg(metrics.weightKg ?? undefined); setSteps(metrics.steps ?? undefined); setLastSyncedAt(new Date().toISOString());
      return true;
    } catch {
  ...
  const addSavedFoodToDay = async (food: SavedFoodSummary, mealType: MealType): Promise<boolean> => {
    if (!user) return false;
    try {
      await saveEntry(user.uid, date, {
        id: crypto.randomUUID(), name: food.name, brand: food.brand, category: food.category, mealType, servings: 1, consumedPercent: 100,
        servingWeightG: food.servingWeightG, ...food.nutrition, hydrationMl: food.hydrationMlPerServing, time: nowTime(), notes: food.notes, source: "database", confidence: "high", sourceFoodId: food.id,
      });
      await loadDaily();
      setNotice(`已加入「${food.name}」到${dateLabel(date)}${food.hydrationMlPerServing > 0 ? `，飲品水分 +${Math.round(food.hydrationMlPerServing)} ml。` : "。"}`);
      return true;
    } catch {
      setNotice("加入常用食物失敗，請再試一次。");
      return false;
    }
  };
...
function EntrySourceBadge({ entry }: { entry: FoodEntry }) {
  if (!entry.source) return null;
  const sourceLabels: Record<string, string> = { nutrition_label: "營養標示", restaurant_official: "店家資料", ingredient_calculation: "食材換算", database: "常用食物", ai_estimated: "AI 估算", manual_estimated: "手動估算" };
  const confidenceLabels: Record<string, string> = { high: "高信心", medium: "中信心", low: "低信心" };
  const sourceLabel = sourceLabels[entry.source] ?? "資料來源";
  const confidenceLabel = entry.confidence ? confidenceLabels[entry.confidence] ?? "未分級" : null;
  return <span className={`entry-source source-${entry.source}`}>{sourceLabel}{confidenceLabel ? `・${confidenceLabel}` : ""}</span>;
}
...
function DailyView({ date, setDate, totals, entries, waterMl, weightKg, steps, loading, lastSyncedAt, savedFoods, setEntryEditor, deleteEntry, saveAsCommon, addWater, quickAddDrink, updateWeight, updateSteps, refresh, exportData }: { date: string; setDate: (value: string) => void; totals: Nutrition; entries: FoodEntry[]; waterMl: number; weightKg?: number; steps?: number; loading: boolean; lastSyncedAt: string | null; savedFoods: SavedFoodSummary[]; setEntryEditor: (entry: EditorState) => void; deleteEntry: (id: string) => Promise<void>; saveAsCommon: (entry: FoodEntry) => Promise<void>; addWater: (amount: number) => Promise<void>; quickAddDrink: (food: SavedFoodSummary) => Promise<void>; updateWeight: (value: string) => Promise<void>; updateSteps: (value: string) => Promise<void>; refresh: () => Promise<void>; exportData: () => void }) {
  ...
  return <>
    ...
    <section className="quick-row body-inputs"><label className="weight-input">體重（kg）<input type="number" inputMode="decimal" min="1" step="0.1" defaultValue={weightKg} key={`${date}-${weightKg}`} onBlur={event => void updateWeight(event.target.value)} /></label><label className="weight-input">步數<input type="number" inputMode="numeric" min="0" step="1" defaultValue={steps} key={`${date}-${steps}`} onBlur={event => void updateSteps(event.target.value)} /></label></section>
...
function Metric({ label, value, target }: { label: string; value: number; target: TargetRange }) { return <div className="ring-wrap"><div className="ring" style={{ background: `conic-gradient(var(--sage) ${Math.min(100, value / target.max * 100)}%,var(--ring-track) 0)` }}><div className="ring-hole"><strong>{Math.round(value)}</strong><span>/{target.min}–{target.max}g</span></div></div><span>{label}</span></div>; }
function DailyGuidance({ totals, waterMl, entries, weightKg }: { totals: Nutrition; waterMl: number; entries: FoodEntry[]; weightKg?: number }) {
  ...
  if (waterMl < targets.waterMl.min) reminders.push(`水分距離理想區間還差 ${Math.max(0, targets.waterMl.min - waterMl)} ml。`);
  if (totals.proteinG < targets.proteinG.min) reminders.push(`蛋白質距離理想區間還差 ${Math.round(targets.proteinG.min - totals.proteinG)} g。`);
  if (entries.length && totals.fiberG < targets.fiberG) reminders.push(`纖維目前 ${formatNutrition(totals.fiberG, "g")}；可補一份蔬果或全穀。`);
  if (totals.sodiumMg > targets.sodiumMg) reminders.push(`鈉已達 ${formatNutrition(totals.sodiumMg, "mg")}；下一餐建議少湯、少醬。`);
...
function Trends({ history }: { history: DailyOverview[] }) {
  ...
  const todaySoFar = days.find(day => day.date === today);
  ...
  const average = (key: keyof Nutrition) => completedDays.length ? completedDays.reduce((sum, day) => sum + Number(day.total[key] ?? 0), 0) / completedDays.length : null;
...
function TargetsSettings({ initial, save }: { initial: HealthTargets; save: (targets: HealthTargets) => Promise<void> }) {
  ...
  type TargetField = { type: "range"; key: RangeTargetKey; label: string; unit: string; hint: string } | { type: "minimum"; key: "fiberG" | "sodiumMg"; label: string; unit: string; hint: string };
  const groups: { title: string; description: string; fields: TargetField[] }[] = [
    { title: "每天的節奏", description: "區間內代表達標；低於或高於時，首頁會直接告訴你差多少。", fields: [{ type: "range", key: "caloriesKcal", label: "每日熱量", unit: "kcal", hint: "減脂的能量預算" }, { type: "range", key: "proteinG", label: "蛋白質", unit: "g", hint: "保留肌肉的重點" }, { type: "range", key: "waterMl", label: "水分", unit: "ml", hint: "包含飲品水分" }] },
    { title: "營養品質", description: "碳水與脂肪使用理想區間；纖維與鈉則以最低／最高值提醒。", fields: [{ type: "range", key: "carbsG", label: "碳水", unit: "g", hint: "每日參考量" }, { type: "range", key: "fatG", label: "脂肪", unit: "g", hint: "每日參考量" }, { type: "minimum", key: "fiberG", label: "纖維至少", unit: "g", hint: "LDL 友善目標" }, { type: "minimum", key: "sodiumMg", label: "鈉上限", unit: "mg", hint: "血壓管理提醒" }] }
  ];
  const updateRange = (key: RangeTargetKey, bound: keyof TargetRange, input: string) => { const value = parseNonNegativeNumber(input); if (value !== null) setValues(current => ({ ...current, [key]: { ...current[key], [bound]: value } })); };
  const updateMinimum = (key: "fiberG" | "sodiumMg", input: string) => { const value = parseNonNegativeNumber(input); if (value !== null) setValues(current => ({ ...current, [key]: value })); };
  const submit = () => { const invalid = (Object.keys(defaultTargets) as (keyof HealthTargets)[]).find(key => typeof values[key] === "object" && values[key].min > values[key].max); if (invalid) return setError("每個區間的最低值不可大於最高值。"); setError(""); setSaving(true); void save(values).finally(() => setSaving(false)); };
  return <><header><div><p className="eyebrow">MY PLAN</p><h1>把方向，放在眼前。</h1><p className="muted">這些是每日參考區間，不必每天完美達成；看一週平均更有意義。</p></div></header><section className="targets-hero"><p>目前健康重點</p><strong>減脂 ・ 改善 LDL ・ 控制血壓</strong><span>每日頁與週報會依這些設定，標示偏低、達標或偏高。</span></section><form className="targets-card" onSubmit={event => { event.preventDefault(); submit(); }}>{groups.map(group => <section className="target-group" key={group.title}><div className="target-group-heading"><h2>{group.title}</h2><p>{group.description}</p></div><div className="targets-grid">{group.fields.map(field => field.type === "range" ? <label className="target-range" key={field.key}><span>{field.label}<small>{field.hint}</small></span><div className="target-range-inputs"><input aria-label={`${field.label}最低值`} type="number" inputMode="decimal" min="0" step="any" value={values[field.key].min} onChange={event => updateRange(field.key, "min", event.target.value)} /><i>至</i><input aria-label={`${field.label}最高值`} type="number" inputMode="decimal" min="0" step="any" value={values[field.key].max} onChange={event => updateRange(field.key, "max", event.target.value)} /><em>{field.unit}</em></div></label> : <label key={field.key}><span>{field.label}<small>{field.hint}</small></span><div><input type="number" inputMode="decimal" min="0" step="any" value={values[field.key]} onChange={event => updateMinimum(field.key, event.target.value)} /><em>{field.unit}</em></div></label>)}</div></section>)}{error && <p className="form-error targets-error">{error}</p>}<button className="save-btn" type="submit" disabled={saving}>{saving ? "儲存中…" : "儲存我的每日目標"}</button></form></>;
}
