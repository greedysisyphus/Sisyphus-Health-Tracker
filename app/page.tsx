...
function TargetsSettings({ initial, save }: { initial: HealthTargets; save: (targets: HealthTargets) => Promise<void> }) {
  ...
  const groups: { title: string; description: string; fields: { key: keyof HealthTargets; label: string; unit: string; hint: string }[] }[] = [
    { title: "每天的節奏", description: "這些數字會出現在每日首頁，幫你安排下一餐。", fields: [{ key: "caloriesKcal", label: "每日熱量", unit: "kcal", hint: "減脂的能量預算" }, { key: "proteinG", label: "蛋白質", unit: "g", hint: "保留肌肉的重點" }, { key: "waterMl", label: "水分", unit: "ml", hint: "包含飲品水分" }] },
    { title: "營養品質", description: "用來提醒纖維、脂肪與鈉是否接近你的健康目標。", fields: [{ key: "carbsG", label: "碳水", unit: "g", hint: "每日參考量" }, { key: "fatG", label: "脂肪", unit: "g", hint: "每日參考量" }, { key: "fiberG", label: "纖維", unit: "g", hint: "LDL 友善目標" }, { key: "sodiumMg", label: "鈉上限", unit: "mg", hint: "血壓管理提醒" }] }
  ];
  const updateValue = (key: keyof HealthTargets, input: string) => { const value = parseNonNegativeNumber(input); if (value !== null) setValues(current => ({ ...current, [key]: value })); };
  const submit = () => { setSaving(true); void save(values).finally(() => setSaving(false)); };
  return <><header><div><p className="eyebrow">MY PLAN</p><h1>把方向，放在眼前。</h1><p className="muted">這些是每日參考值，不必每天完美達成；看一週平均更有意義。</p></div></header><section className="targets-hero"><p>目前健康重點</p><strong>減脂 ・ 改善 LDL ・ 控制血壓</strong><span>每日頁與週報會依這些設定，提供更貼近你的提醒。</span></section><form className="targets-card" onSubmit={event => { event.preventDefault(); submit(); }}>{groups.map(group => <section className="target-group" key={group.title}><div className="target-group-heading"><h2>{group.title}</h2><p>{group.description}</p></div><div className="targets-grid">{group.fields.map(field => <label key={field.key}><span>{field.label}<small>{field.hint}</small></span><div><input type="number" inputMode="decimal" min="0" step="any" value={values[field.key]} onChange={event => updateValue(field.key, event.target.value)} /><em>{field.unit}</em></div></label>)}</div></section>)}<button className="save-btn" type="submit" disabled={saving}>{saving ? "儲存中…" : "儲存我的每日目標"}</button></form></>;
}
