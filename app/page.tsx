...
import { getBodyLog, getDailyLog, getUserProfile, listDailyEntries, listDailyOverviews, listFoods, removeEntry, saveBodyLog, saveEntry, saveWater, type DailyOverview, type SavedFoodSummary } from "../services/health-service";
...
export default function Home() {
  ...
  const exportRecords = async (startDate: string, endDate: string) => {
    ...
    const dailyRecords = allDays.filter(day => day.date >= startDate && day.date <= endDate).sort((a, b) => a.date.localeCompare(b.date)).map(day => ({
      ...
      foods: day.entries.map(entry => ({ name: entry.name, meal: entry.meal, portion: entry.portion, unit: entry.unit, nutrition: { calories_kcal: entry.calories, protein_g: entry.protein, carbohydrate_g: entry.carbs, fat_g: entry.fat, sugar_g: entry.sugar, fiber_g: entry.fiber, saturated_fat_g: entry.saturatedFat, sodium_mg: entry.sodium }, source: entry.source, confidence: entry.confidence, notes: entry.notes ?? null })),
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      </> : view === "overview" ? <Overview history={history} openDate={(nextDate) => { setDate(nextDate); setView("daily"); }} exportData={() => setExporting(true)} /> : view === "trends" ? <Trends history={history} /> : <FoodLibrary foods={savedFoods} />}
    </section>
...
function ExportSheet({ close, exportRecords }: { close: () => void; exportRecords: (startDate: string, endDate: string) => Promise<void> }) { const today = formatLocalDate(new Date()); const [startDate, setStartDate] = useState(formatLocalDate(new Date(new Date(`${today}T12:00:00`).getTime() - 6 * 86400000))); const [endDate, setEndDate] = useState(today); const [working, setWorking] = useState(false); const download = async () => { if (startDate > endDate || working) return; setWorking(true); try { await exportRecords(startDate, endDate); } finally { setWorking(false); } }; const copyPrompt = async () => { await navigator.clipboard.writeText("我會附上一份健康紀錄 JSON。請依資料來源與 confidence 判讀，分析熱量、蛋白質、纖維、鈉、飲水、體重與步數趨勢；指出資料缺漏，不要把估算當成精確值，並給我 3 個具體可執行的建議。我的目標是減脂、改善 LDL、控制血壓與保留肌肉。"); }; const copyHermes = async () => { await navigator.clipboard.writeText(`/health-log 請分析 ${startDate} 到 ${endDate} 的健康紀錄。先用 get_range_summary 讀取資料，再比較熱量、蛋白質、纖維、鈉、飲水、體重與步數，說明缺漏資料與估算的限制，最後給我 3 個具體建議。`); }; return <div className="sheet-backdrop" onClick={close}><section className="sheet export-sheet" onClick={event => event.stopPropagation()}><div className="grab" /><div className="sheet-title"><div><p className="eyebrow">EXPORT</p><h2>匯出給 GPT 分析</h2></div><button onClick={close}>×</button></div><p className="muted export-copy">下載的是結構化 JSON，包含食物明細、營養總計、飲水、體重、步數與資料可信度。</p><div className="form-grid"><label>開始日期<input type="date" value={startDate} onChange={event => setStartDate(event.target.value)} /></label><label>結束日期<input type="date" value={endDate} onChange={event => setEndDate(event.target.value)} /></label></div><button className="save-btn" onClick={() => void download()} disabled={working || startDate > endDate}>{working ? "準備中…" : "下載 JSON"}</button><button className="parse-btn" onClick={() => void copyPrompt()}>複製 GPT 分析提示</button><button className="parse-btn" onClick={() => void copyHermes()}>複製 Hermes 分析指令</button><p className="muted export-copy">下載後把 JSON 檔附到 GPT，再貼上分析提示；或把 Hermes 指令貼到 Discord。</p></section></div> }
